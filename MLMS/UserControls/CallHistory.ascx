        ...
        if (memberID != "") {
            if (date.length > 0) {
                $.ajax({
                    ...
                    data: '{ "LeadID":"' + memberID + '", "Date":"' + $('#<%=txtBxCallLogDate.ClientID %>').val() + '", "Time":"' + $('#<%=ddlCallStart.ClientID %>').val() + '", "Notes":"' + encodeURIComponent($('#<%=txtBxCallNotes.ClientID %>').val()) + '"}',
        ...
        if (btnText == "Edit") {
            ...
            if (memberID != "") {
                $.ajax({
                    ...
                    data: '{ "MemberCallLog":"' + memberID + '", "Notes":"' + encodeURIComponent(notes) + '"}',
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    success: function (data) {
                        CancelCallHistory();
